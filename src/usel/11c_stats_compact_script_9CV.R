...
x<-readRDS(paste0(path_results,"9CV_4models_13pred.rds"))
...
tstat<-lapply(x,function(be){
  tstat <- compRegrTests( be@model$glmboost_ffs)
  ...
  mr<-lapply(unique(be$tstat$model_response), function(mr){
    ...
    be_stats<- lapply(unique(be$tstat$sample), function(s){
      ...
      data.frame(be = substr(be$tstat$model_selector[1], 1, 3), 
                 response = dt$model_response[3], #auf 1 bei mehreren responses
...
mstat <- lapply(x, function(be){
  mod_r <- lapply(seq(length(be@model$glmboost_ffs)), function(r){
    mod_s <- lapply(seq(length(be@model$glmboost_ffs[[r]])), function(s){
      if(class(be@model$glmboost_ffs[[r]][[s]]$model) == "try-error"){
        df <- NULL
      } else {
        df <- data.frame(be = substr(be@model$glmboost_ffs[[r]][[s]]$training$SELECTOR[1], 1, 3), 
                         response = be@model$glmboost_ffs[[r]][[s]]$response,
                         r_squared = be@model$glmboost_ffs[[r]][[s]]$model$results$Rsquared,
                         #r_squared_sd = be@model$glmboost_ffs[[r]][[s]]$model$results$RsquaredSD,
                         rmse = be@model$glmboost_ffs[[r]][[s]]$model$results$RMSE)
                         #rmse_sd = be@model$glmboost_ffs[[r]][[s]]$model$results$RMSESD)
      }
      return(df)
    })
...
saveRDS(errors,paste0(path_stats,"allnewstats_GLM_9CV_allRESP.rds"))
