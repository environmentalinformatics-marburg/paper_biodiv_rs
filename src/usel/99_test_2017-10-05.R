...
test_model <- trainModel(x = veg_re_g_gpm_indv[["AEG"]],
                         n_var = NULL,             
...
tstat = compRegrTests(models = test_model@model[[1]], per_model = TRUE, 
...
mstat = lapply(test_model@model$pls_ffs[[1]], function(s){
...
ggplot(data = errors[errors$smpl %in% seq(10) & errors$variable == "rmse",], 
...
ggplot(data = errors[errors$smpl %in% seq(10) & errors$variable == "r_squared",], 
       aes(x = response, y = value, color = stat)) + 
  geom_boxplot() + 
