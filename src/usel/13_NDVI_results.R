source("F:/Ulli/exploratorien/scripts/project_biodiv_rs/src/usel/00_b_set_environment_start_modeltrain.R")

# ----------------------read data products
x<-readRDS(paste0(path_results,"NDVI_pls_ffs_9CV.rds"))

  #frequent used predictors
pred_freq<-readRDS(paste0(path_stats,"pred_frequency_PLS_NDVI.rds"))
...
pls_NDVI<-ggplot(data= plsNDVI[plsNDVI$smpl %in% seq(10) & plsNDVI$variable=="rmse"& plsNDVI$response %in% mr_all ,],
  ...
  labs(title = "Model performance of PLS NDVI",
...
pls<-ggplot(data= pls[pls$smpl %in% seq(10) & pls$variable=="rmse"& pls$response %in% mr_all ,],
                 aes( x= be ,y=value, fill=stat),shape=21,
                 stroke=5)+
  geom_boxplot(lwd=0.2)+
...
grid.arrange(plsNDVI,pls, ncol=2)
...
pls_obs_NDVI<-ggplot(data=pls_obs_pred_NDVI[pls_obs_pred_NDVI$response %in% mr_all ,],
  ...
  labs(title = "distribution of observed values and validated values after PLS NDVI regression",
       lineheigth=2,
...
pls_obs<-ggplot(data=pls_value_pred_test[pls_value_pred_test$response %in% mr_all ,],
                     aes( x= be ,y=value, fill=variable), size=1.5)+
  scale_fill_manual(values=wes_palette(n=3,name="Chevalier"), labels=c("validated","observed"))+
  geom_boxplot(position= position_dodge(width=0.6))+
...
#-- some statistics
cor.test(val_obs_PLS$testing_predicted[val_obs_PLS$be=="AEG"],val_obs_PLS$testing_response[val_obs_PLS$be=="AEG"])$estimate
summary(lm(testing_predicted~testing_response, data=val_obs_PLS[val_obs_PLS$be=="AEG",]))
