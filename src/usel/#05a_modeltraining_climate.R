...
# Predict dataset --------------------------------------------------------------
if(compute){
  adf_clim_g_gpm <- readRDS(file = paste0(path_rdata, "veg_meta_g_gpm.rds"))
#why detecting the cores when we dont use it?  
  ...
  saveRDS(adf_clim_g_gpm, file = paste0(path_rdata, "adf_rs_gpm_trainModel.rds"))
} else {
